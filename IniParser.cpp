...
void IniParser::Initialize(const char *filename_cstr)  throw (exc_io){
    ...
    if(file.good()) {
        ...
        while (!file.eof()){
            if(lineOfIniFile[0] == ';' || lineOfIniFile == "") {
                getline(file,lineOfIniFile);
                continue;
            }
            if (lineOfIniFile[0] == '[') {
    ...
    else throw exc_io("EXCEPTION: \"File not found!\"");
}
...
bool IniParser::IsHaveParam(const char *section_name, const char *param_name) const throw(exc_ini_not_initied, exc_ini_not_found) {
    if(dataStore.empty()){
        throw exc_ini_not_initied("EXCEPTION: \"File is not initialized\"");
    }
...
void IniParser::print() const{
    ...
    for (it = dataStore.begin(); it != dataStore.end(); ++it) {
        cout << it->first << endl;
        map<std::__cxx11::basic_string<char, std::char_traits<char>, std::allocator<char>>, std::__cxx11::basic_string<char, std::char_traits<char>, std::allocator<char>>>::const_iterator it2;
        for (it2 = dataStore.at(it->first).begin(); it2 != dataStore.at(it->first).end(); it2++) {
